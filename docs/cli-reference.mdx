...
`RUNNER_TEMP` to an existing, bounded regular file with no symlink or hard-link
ambiguity. This also supports runner-created files mounted into GitHub Actions
containers under a different numeric user ID. GrantTrace does not create,
truncate, or follow links for this file. Terminal output still uses the selected
