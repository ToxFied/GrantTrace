...
| `granttrace scenario inspect NAME`  | Inspect one recording's permissions, routes, and evidence |
...
| `granttrace help COMMAND`           | Show help for one command                                 |
| `granttrace --version`              | Print the installed GrantTrace version                    |

Run `granttrace <command> --help` or `granttrace help <command>` for complete
usage. Close command-name typos produce one safe suggestion without reflecting
the original argument back to the terminal.
...
granttrace scenario inspect <name>
granttrace scenario remove <name>
```

`list` shows saved local recordings in name order. `inspect` analyzes one
recording with the current pinned catalog and shows its selected permissions,
resolved route templates, and evidence sources. Repeated requests count as
observations; each resolved route appears once.

Inspection is read-only and works in CI. It does not rerun the scenario, access
GitHub, or change observations or `granttrace.lock.json`. Its selection uses
the default policy for that recording alone, without the accepted contract's
frontier choice or manual keeps. Use `check` for aggregate contract review.

Missing, empty, malformed, or unsafe recordings exit `5`. A recording whose
scenario names do not match its filename also exits `5`; `list` rejects such
recordings too. Unknown or contradictory evidence exits `7` with an explanation.

`remove` deletes only the local recording. Run `check` afterward to review the
coverage change before accepting it.
...
Interactive TTY output uses restrained ANSI color for status, headings, and
permission levels. Piped output, CI, JSON, and Markdown remain deterministic
plain text without ANSI styling. Set `NO_COLOR=1` to force plain terminal
output.
